...
const Indicator = ({
  ...
  const stylez = useAnimatedStyle(() => {
    ...
    const transform = itemsLayout.length > 1 ? [{
      ...
      itemsLayout.map(v => isRTL ? -1 * v.x - 8 : v.x === v.width ? v.x-16 : v.x === v.width * 2 ? v.x - 24 :  v.x - 8))
    }] : undefined;
    const width = itemsLayout.length > 1 ? interpolate(indexDecimal.value, itemsLayout.map((_, i) => i), itemsLayout.map(v => v.width - 16)) : (_itemsLayout$ = itemsLayout[0]) === null || _itemsLayout$ === void 0 ? void 0 : _itemsLayout$.width;
    return {
      transform,
      width,
